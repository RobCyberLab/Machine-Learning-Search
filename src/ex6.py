...
def search_lsh(assign, student, threshold, lsh_list):
    ...
    # Lista pentru a stoca rezultatele găsite
    results = []

    # Iterăm prin candidați și calculăm similaritatea pentru fiecare
    for candidate in candidates:
        assign_candidate, student_candidate = candidate.split('_')
        doc_content = get_content(assign_candidate, student_candidate)
        if doc_content is None:
            continue  # Sărim dacă conținutul documentului candidat nu este găsit

        doc_sorted_content = sorted(doc_content)
        doc_hashes = compute_bands(doc_sorted_content)

        # Calculăm similaritatea între documentul de referință și documentul candidat
        similarity = jaccard_similarity(set(hashes), set(doc_hashes))

        # Verificăm dacă similaritatea este diferită de 0.0 înainte de a adăuga rezultatul în listă
        if similarity != 0.0:
            # Adăugăm rezultatul în lista de rezultate
            results.append((assign_candidate, student_candidate, similarity))

    # Verificăm dacă avem rezultate pentru a le afișa
    if results:
        # Sortăm rezultatele în ordine descrescătoare a similarității
        results_sorted = sorted(results, key=lambda x: x[2], reverse=True)

        # Printăm rezultatele sortate
        for result in results_sorted:
            print("Found similar file:", result[0], result[1], "with similarity:", result[2])
    else:
        print("No similar files found.")

    print(results_sorted)
    # Returnăm numărul total de fișiere similare găsite
...
def create_similarity_file(lsh_list):
    try:
        with open("similarity_file.txt", "w") as file:
            for i, (id1, bands1) in enumerate(lsh_list):
                for id2, bands2 in lsh_list[i+1:]:
                    similarity = jaccard_similarity(set(bands1), set(bands2))
                    if similarity != 0.0:
                        file.write(f"{id1} - {id2}: Similarity {similarity}\n")
        print("Similarity file created successfully.")
    except Exception as e:
        print(f"Error creating similarity file: {e}")
...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            create_similarity_file(lsh_list)
